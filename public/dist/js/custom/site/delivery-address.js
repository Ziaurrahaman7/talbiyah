    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(result, function (key, value) {
                $("#country").append(
                    `<li class="country-name cursor-pointer py-3 primary-bg-hover hover:text-gray-12 px-2.5" data-country="${value.code}" data-value="${value.code}">${value.name}</li>`
                );
            });
...
function reloadClick() {
    $(".country-name").click(function () {
        ...
        $(".view-country")
            ...
            .html(
                country +
                    `<img class="mx-2 my-1 neg-transition-scale" src="${SITE_URL}/frontend/assets/img/product/arrow-right.svg" alt="">`
            );
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $.each(result.data, function (key, value) {
                    $("#state").append(
                        `<li class="state-name cursor-pointer py-3 primary-bg-hover hover:text-gray-12 px-2.5" data-state="${value.code}">${value.name}</li>`
                    );
                });
                ...
                if (result.length <= 0 && result.data.length <= 0) {
                    ...
                    $("#state").html(
                        `<li class="state-name cursor-pointer py-3 primary-bg-hover hover:text-gray-12 px-2.5">${errorMsg}</li>`
                    );
                }
                reloadClick();
            },
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $.each(res.data, function (key, value) {
                    $("#city").append(
                        `<li class="city-name cursor-pointer py-3 primary-bg-hover hover:text-gray-12 px-2.5" data-city="${value.name}">${value.name}</li>`
                    );
                });
                ...
                if (res.length <= 0 && res.data.length <= 0) {
                    ...
                    $("#city").html(
                        `<li class="city-name cursor-pointer py-3 primary-bg-hover hover:text-gray-12 px-2.5">${errorMsg}</li>`
                    );
                }
                reloadClick();
            },
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data["status"] == 1) {
                ...
                $("#shippingAmount").text(
                    decimalNumberFormatWithCurrency(data["amount"])
                );
            } else {

...
let loader = `<option value="">${jsLang("Loading")}...</option>`,
    selectCity = `<option value="">${jsLang("Select City")}</option>`,
    selectState = `<option value="">${jsLang("Select State")}</option>`;
var selectedCountry = "",
    selectedState = "",
    selectedCity = "";
    ...
    $(".country-name").click(function () {
        ...
        $(".city-name").on("click", function () {
            (city = $(this).text()),
                ...
                $(".view-city , .view-state , .view-country").addClass(
                    "hidden"
                ),
                getShipping();
        }),
...
function getState(country_code) {
    country_code
        ? ($("#stateHtml").html(""),
          "null" == oldCity && $("#city").html(selectCity),
          $.ajax({
              url:
                  SITE_URL +
        ...
        : ($("#state").html(selectState), $("#city").html(selectCity));
}
    ...
    siso && ciso
        ? ($("#city").html(""),
          $.ajax({
              ...
              dataType: "json",
              beforeSend: function () {
                  $("#city").html(loader), $("#city").attr("disabled", "true");
              },
        ...
        success: function (data) {
            1 == data.status
                ...
                : ($("#shipping-name").text(jsLang("Not Applicable")),
                  $("#shippingAmount").text(
                      decimalNumberFormatWithCurrency(0)
                  ));
        },
    });
}
$(".change-toggle-button").click(function (e) {
    ...
    $(".search-name-country").on("keyup", function () {
        ...
        $(".list-country li")
            .show()
            .filter(function () {
                return -1 == $(this).text().toLowerCase().trim().indexOf(value);
            })
            .hide();
    }),

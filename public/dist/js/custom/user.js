"use strict";
if (
    $(".main-body .page-wrapper").find("#user-add-container").length ||
    $(".main-body .page-wrapper").find("#user-edit-container").length
) {
...
if ($(".main-body .page-wrapper").find("#user-import-container").length) {
    $("#fileRequest").on("click", function () {
        window.location =
            SITE_URL.replace("/admin", "") +
            "/dist/downloads/user_sheet.csv";
    });
    ...
    $("#validatedCustomFile").on("change", function () {
        ...
        if ($.inArray(ext, ["csv"]) == -1) {
            ...
            $("#note_txt_2").html(
                '<h6> <span class="text-danger font-weight-bolder">' +
                    jsLang("Invalid file extension") +
                    ' </span> </h6> <span class="badge badge-info note-style">' +
                    jsLang("Note") +
                    '</span><small class="text-info"> ' +
                    jsLang("Allowed File Extensions: csv")
            ) + "</small>";
        } else {
...
function passwordValidation() {
    ...
    if (
        ...
        $(".password-validation-error")
            .addClass("text-red")
            .text(jsLang("Passwords does not match!"));
        return false;
    }
